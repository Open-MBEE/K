...
case class Model(
  packageName: Option[PackageDecl], 
...
trait TopDecl 
  ...
  case class TypeDecl(ident: String, typeParams: List[TypeParam], ty: Option[Type]) extends MemberDecl 

  case class PropertyDecl(
...
trait Exp 
  ...
  case class MatchCase(patterns: List[Pattern], exp: Exp) extends Exp // How did this become an expression?
...
trait Argument extends Exp // How come this became an expression?
