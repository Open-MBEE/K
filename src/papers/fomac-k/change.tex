...
of all events is greater than zero (\code{req}
...
question. Given another event \name{e}, it is now possible to specify
if the event occurs \name{before}, \name{after}, or is
\name{contained} by the current event. We do not include the other
...
must occur before the \name{commWindow} and at least one science
